...
{
    public class ForceGSSwapAerial : INetMessage
    {
        NetworkInstanceId IDNet;
        NetworkInstanceId enemyIDNet;

        public ForceGSSwapAerial()
        {

        }

        public ForceGSSwapAerial(NetworkInstanceId IDNet, NetworkInstanceId enemyIDNet)
        {
            this.IDNet = IDNet;
        ...
        public void OnReceived()
        {
            if (NetworkServer.active)
            {
                ...
                foreach (EntityStateMachine statemachine in statemachines)
                {
                    ...
                    {
                        statemachine.SetState(new GreatswordSwapAerial
                        {
